...
radio <- read.csv("Survey_results_final.csv", header = TRUE,
                  stringsAsFactors = F, na.strings = c("", "NA", "select one"))
```
...
* missing age or incorrectly formatted ("female, Eighteen") or invalid ("223")
  * for missing age, do not take observation into consideration
  * for incorrectly formatted values, change the ones that can be turned to integers
  * for invalid values, do not take that observation into consideration
* missing education ("select one option")
  * for missing education, do not take observation into consideration
...
i. Does this sample appear to be a random sample from the general population of the USA? No way of knowing, but we assume it to be. 
ii. Does this sample appear to be a random sample from the MTURK population? No way of knowing, but we assume it to be.
...
results
```
Yes, this interval does capture the true $\beta_1 = 0.6$
### Q2.5
...
RSE is 2.071, 0.071 above $\sigma = 2$
### Q2.6
...
```
Yes, year is a significant variable at the .05 level our P value < 0.0001
...
```
Yes, it is significant. For every increase increase in year, holding horsepower constant, mpg increases by 2.192
...
```
Yes, cylinders is significant at the 0.01 level. For every increase in cylinders, holding horsepower constant, we have a decrease in mpg of 0.08612
...
```
Yes, cylinders are significant at the 0.01 level only for 4 cylinders but not for 5,6,8.  anova() calculates the type 1 variance whereas Anova() calculates type 2 and type 3 variance.
