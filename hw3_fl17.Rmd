...
pacman::p_load(bestglm, pROC, leaps, car, tidyverse, mapproj, caret, xtable)
...
data.frame(na_count)
bill.train <- na.omit(bill) #Should we not omit???
...
bill.train = bill.train[,-c(1, 4, 11, 13)]  #Removing Bill ID as it wouldn't make much of a difference. Remove status and number sponsors which are linearly dependent
summary(bill.train) #Draw graphs??? 
str(bill.train)
```
...
Anova(bill.fit7)
#anova(bill.fit1,  bill.fit7,  test="Chisq") How to interpret this???
```
...
data.frame(na_count)
bill.test <- na.omit(bill.test) #Should we not omit???
...
* Exhautive search hanged.
* Preformed backward model selection by eliminating the variables with highest p-value. Used Anova to determine the effect of categorical variable: all variables are significant at 0.001 level
* Used 
