...
In the wordcloud, the larger the size of the word, greater is its coefficient, and hence higher is the correspoding probability of the rating being high. From the plot, we can see that the words excel (excellent), amaz (amazing), delici (delicious), perfect, yum all have big font sizes, and hence if the text contains these words, the rating is likely to be higher.
...
In the wordcloud, the larger the size of the word, greater is its negative coefficient, and hence higher is the correspoding probability of the rating being low. From the plot, we can see that the words worst, rude, terribl (terrible), , bland, overpr(overpriced), all have big font sizes, and hence if the text contains these words, the rating is likely to be low.


#### (iv) Summarize the findings. 

The sentiment analysis has done a great job of distinguishing between the positive and negative words. By visualizing the top 100 words in wordcloud, we can verify that the words and their relative sizes do make a lot of sense.
...
testerror.glm
```
The testing error using logistic egression is `0.2074`.
...
The testing errors obtained from 5.ii and this are very similar, but not excatly the same. In 5.ii we obtained a testing error of `0.2074`, while using RTextTools we obtained `0.2316`.
...
If we were given a review in order to predict a rating, we would first do the same preprocessing to the text as we did here. Then we would create a Document-Term frequency matrix. Then, using the final model developed, we would predict the rating.
